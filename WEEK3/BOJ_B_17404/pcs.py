...
for f in range(3):
    ...
    for i in range(1, n):  # 2번집 ~ 마지막 집 칠하기
        dp[i][0] = min(dp[i-1][1], dp[i-1][2]) + cost[i][0]  # 현재 집 R(빨간색) - 이전 집(초록 or 파랑)
        dp[i][1] = min(dp[i-1][0], dp[i-1][2]) + cost[i][1]  # 현재 집 G(초록색) - 이전 집(빨강 or 파랑)
        dp[i][2] = min(dp[i-1][0], dp[i-1][1]) + cost[i][2]  # 현재 집 B(파란색) - 이전 집(빨강 or 초록)
    for i in range(3):
        if i == f:  # 마지막 집 색 != 첫번째 집 색
            continue
